// Variables qui récupère la div class gallery et filter
...
// Fonction pour créer les balises du DOM dynamiquement
    ...
    icon.addEventListener("click", async (e) => {
      ...
      try {
        const reponse = await fetch(
          "http://localhost:5678/api/works/" + iconId,
          deletePic
        );
        if (reponse.ok) {
          const data = reponse;
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (reponse.ok) {
      ...
      console.error("Erreur lors de l'ajout :", reponse.status);
    }
  } catch (error) {
